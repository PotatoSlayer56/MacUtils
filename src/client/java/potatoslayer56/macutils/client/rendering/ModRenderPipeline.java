package potatoslayer56.macutils.client.rendering;
...
public class ModRenderPipeline implements ClientModInitializer {
  private static ModRenderPipeline instance;
  ...
  private static final RenderPipeline FILLED_THROUGH_WALLS = RenderPipelines.register(RenderPipeline.builder(RenderPipelines.DEBUG_FILLED_SNIPPET)
    .withLocation(Identifier.fromNamespaceAndPath(MacUtils.MOD_ID, "pipeline/debug_filled_box_through_walls"))
  ...
  // :::custom-pipelines:drawing-phase
  public static ModRenderPipeline getInstance() {
    return instance;
  }
  ...
  private void renderWaypoint(LevelRenderContext context) {
    ...
    renderFilledBox(matrices.last().pose(), buffer, 0f, 100f, 0f, 1f, 101f, 1f, 0f, 1f, 1f, 1f);
  ...
  private GpuBuffer upload(MeshData.DrawState drawParameters, VertexFormat format, MeshData builtBuffer) {
    ...
    if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
      ...
      vertexBuffer = new MappableRingBuffer(() -> MacUtils.MOD_ID + " example render pipeline", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_MAP_WRITE, vertexBufferSize);
    }
  ...
  private static void draw(Minecraft client, RenderPipeline pipeline, MeshData builtBuffer, MeshData.DrawState drawParameters, GpuBuffer vertices, VertexFormat format) {
    ...
    try (RenderPass renderPass = RenderSystem.getDevice()
      ...
      .createRenderPass(() -> MacUtils.MOD_ID + " example render pipeline rendering", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty())) {
